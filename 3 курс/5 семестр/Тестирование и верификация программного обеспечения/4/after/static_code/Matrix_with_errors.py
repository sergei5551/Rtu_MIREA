class Matrix:
    def __init__(self, matrix_v=[], size="1x1"):  # Pylint: Опасное значение по умолчанию
        self.matrix_v = matrix_v
        if len(matrix_v) > 0:
            self.size = f"{len(matrix_v)}x{len(matrix_v[0])}"
        else:
            self.size = size
    unused_var = 42  # Pylint: Неиспользуемая переменная

    def change_element(self, index_row, index_col, element):
        if (index_row >= len(self.matrix_v) or
        index_col >= len(self.matrix_v[0])):  # PyCodeStyle: Неправильный отступ
            raise IndexError(
                f"Индекс [{index_row}][{index_col}] вне границ матрицы {self.size}"  # PyCodeStyle: Слишком длинная строка
            )
        self.matrix_v[index_row][index_col] = element
        x=5  # PyCodeStyle: Отсутствуют пробелы вокруг оператора
    ...
    def sum(matrix1, matrix2) -> "Matrix":
        ...
        # Radon: Высокая сложность из-за вложенных условий
        if matrix1.matrix_v:
            if matrix2.matrix_v:
    ...
    def multiplication(matrix1, matrix2) -> "Matrix":
        ...
        if len(matrix1.matrix_v[0]) != len(matrix2.matrix_v):
            raise ValueError(
                f"Несовместимые размеры для умножения: {matrix1.size} и {matrix2.size}"
            )
        ...
        for i in range(len(matrix1.matrix_v)):
            for j in range(len(matrix1.matrix_v[0])):
                if not isinstance(matrix1.matrix_v[i][j], (int, float)):
                    raise TypeError(
                        f"Элемент matrix1[{i}][{j}] = {matrix1.matrix_v[i][j]} не является числом"
                    )
        ...
        for i in range(len(matrix2.matrix_v)):
            for j in range(len(matrix2.matrix_v[0])):
                if not isinstance(matrix2.matrix_v[i][j], (int, float)):
                    raise TypeError(
                        f"Элемент matrix2[{i}][{j}] = {matrix2.matrix_v[i][j]} не является числом"
                    )
