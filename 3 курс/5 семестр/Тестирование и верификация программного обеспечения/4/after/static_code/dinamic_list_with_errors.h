#pragma once
#include <iostream>
#include <cstring>  // Cppcheck: Неиспользуемый заголовок
...
class List{
 ...
 private:
  ...
  int Size;
  Node<T> *head;
  char buffer[100];  // FlawFinder: Фиксированный буфер
...
List<T>::List() {
  Size = 0;
  head = nullptr;
  strcpy(buffer, "test");  // FlawFinder: Небезопасная функция
}
...
T& List<T>::operator[](const int index) {
  ...
  // Cppcheck: Возврат ссылки на несуществующий объект
  static T dummy;
  return dummy;
}
...
void List<T>::task3() {
  ...
  // Cpplint: Длинная строка комментария которая определенно превышает лимит в 80 символов
}